...
extern "C" __declspec(dllexport) VOID AmsiProfiling(PVOID rcx, PVOID rdx, PVOID r8, PVOID r9, PVOID rsp) {
	PROLOGX64(rsp);


	// WRITE YOUR CODE HERE
	DWORD64 time_elapsed = (DWORD64)r9;
	double msElapsed = time_elapsed / 10000.0;

	std::wstringstream ss;
	ss << std::fixed << std::setprecision(3) << msElapsed;
	std::wstring ws = ss.str();

	Log(L"function consumes %sms\n", ws.c_str());
	// HOOK CODE END
