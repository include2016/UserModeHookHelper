...
// Instant Hook List persistence
// Format: PROCESS_FNV_HASH:DLL_FNV_HASH:HOOKSEQ_PATH
bool RegistryStore::ReadInstantHookList(std::vector<std::tuple<unsigned long long, unsigned long long, std::wstring>>& outEntries) {
	outEntries.clear();
	HKEY hKey = NULL;
	LONG r = RegOpenKeyExW(HKEY_LOCAL_MACHINE, REG_PERSIST_SUBKEY, 0, KEY_READ, &hKey);
	if (r != ERROR_SUCCESS) return true; // treat missing as empty
	DWORD type = 0, dataSize = 0;
	r = RegQueryValueExW(hKey, INSTANT_HOOK_LIST_NAME, NULL, &type, NULL, &dataSize);
	if (r != ERROR_SUCCESS) { RegCloseKey(hKey); if (r == ERROR_FILE_NOT_FOUND) return true; return false; }
	if (type != REG_MULTI_SZ) { RegCloseKey(hKey); return false; }
	if (dataSize == 0) { RegCloseKey(hKey); return true; }
	std::vector<wchar_t> buf(dataSize / sizeof(wchar_t));
	r = RegQueryValueExW(hKey, INSTANT_HOOK_LIST_NAME, NULL, NULL, reinterpret_cast<LPBYTE>(buf.data()), &dataSize);
	RegCloseKey(hKey);
	if (r != ERROR_SUCCESS) return false;
	...
	while (idx < wcCount) {
		...
		// Parse: PROCESS_FNV:DLL_FNV:HOOKSEQ_PATH
		...
		std::wstring hookSeqPath = line.substr(c2 + 1);
		...
		outEntries.emplace_back(procHash, dllHash, hookSeqPath);
	}
	return true;
}

bool RegistryStore::WriteInstantHookList(const std::vector<std::tuple<unsigned long long, unsigned long long, std::wstring>>& entries) {
	std::vector<wchar_t> buf;
	for (auto &e : entries) {
	...
	LONG r = RegCreateKeyExW(HKEY_LOCAL_MACHINE, REG_PERSIST_SUBKEY, 0, NULL, REG_OPTION_NON_VOLATILE, KEY_WRITE, NULL, &hKey, &disp);
	if (r != ERROR_SUCCESS) return false;
	LONG rr = RegSetValueExW(hKey, INSTANT_HOOK_LIST_NAME, 0, REG_MULTI_SZ, reinterpret_cast<const BYTE*>(buf.data()), (DWORD)(buf.size() * sizeof(wchar_t)));
	RegCloseKey(hKey);
	return rr == ERROR_SUCCESS;
}

bool RegistryStore::AddInstantHookEntry(unsigned long long processFnvHash, unsigned long long dllFnvHash, const std::wstring& hookSeqPath) {
	...
	// Check if entry already exists (by processFnvHash)
	for (auto &e : entries) {
		if (std::get<0>(e) == processFnvHash) return true; // Already exists
	}

	entries.emplace_back(processFnvHash, dllFnvHash, hookSeqPath);
	return WriteInstantHookList(entries);
}

bool RegistryStore::RemoveInstantHookEntry(unsigned long long processFnvHash) {
	std::vector<std::tuple<unsigned long long, unsigned long long, std::wstring>> entries;
	if (!ReadInstantHookList(entries)) return false;
	...
	if (out.empty()) {
		...
		if (r == ERROR_SUCCESS) {
			RegDeleteValueW(hKey, INSTANT_HOOK_LIST_NAME);
			RegCloseKey(hKey);
		}
		return true;
	}

	return WriteInstantHookList(out);
}
