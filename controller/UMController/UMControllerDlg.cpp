	...
	bool InjectTrampoline(DWORD targetPid, const wchar_t* fullDllPath) override {
		...
		if (timeoutMs > 0) {
			...
			if (waitResult == WAIT_OBJECT_0) {
				LOG_CTRL_ETW(L"InjectTrampoline: confirmed success pid=%u\n", targetPid);
				return true;
			}
		...
		for (int iter = 0; iter < maxIterations; ++iter) {
			if (GetModuleBase(targetPid, trampName.c_str(), &trampoline_dll_base) && trampoline_dll_base != 0) {
				LOG_CTRL_ETW(L"InjectTrampoline: detected via polling after %d iterations\n", iter + 1);
				return true;
			}
			Sleep(100);
		}

		LOG_CTRL_ETW(L"InjectTrampoline: polling timeout pid=%u\n", targetPid);
		return false;
	}
...
BOOL CUMControllerDlg::OnInitDialog()
{
	...
	// Restore instant hook listeners from registry
	try {
		std::vector<std::tuple<unsigned long long, unsigned long long, std::wstring>> instantHooks;
		if (RegistryStore::ReadInstantHookList(instantHooks)) {
...
void CUMControllerDlg::OnNMRClickListProc(NMHDR *pNMHDR, LRESULT *pResult)
{
	...
	if (instantHookNtPath.empty()) {
		...
		if (it != m_SessionNtPathCache.end()) instantHookNtPath = it->second;
	}
	if (!instantHookNtPath.empty()) {
	...
	bool marked = false;
	std::wstring ntpath2 = item.path;
	if (ntpath2.empty()) {
		ProcKey key{ pid, item.startTime };
		auto it = m_SessionNtPathCache.find(key);
		if (it != m_SessionNtPathCache.end()) ntpath2 = it->second;
	}
	if (!ntpath2.empty()) {
		std::wstring low = ntpath2;
		for (wchar_t &c : low) c = towlower(c);
	...
	// Instant Hook requires process to be in hook list
...
void CUMControllerDlg::OnSetInstantHook()
{
	...
	// 3. Check if instant hook is already enabled
	...
	// 4. Pop up CFileDialog to select .hookseq file
	...
	// 5. Parse hookseq to get target DLL list
	std::vector<InstantHookManager::HookTarget> targets;
	if (!InstantHookManager::ParseHookSeqFile(hookSeqPath.c_str(), targets)) {
	...
	// 7. For each target DLL, add to InstantHookManager with dllFnvHash
	for (auto& t : targets) {
		t.targetPid = pid;
		...
		// Persist to registry for each target
		RegistryStore::AddInstantHookEntry(processFnvHash, t.dllFnvHash, hookSeqPath);
		m_InstantHookMgr->AddTarget(t);
	}

	// 8. Start all listeners
	m_InstantHookMgr->StartAllListeners();

	MessageBox(L"Instant Hook enabled successfully.", L"Success", MB_ICONINFORMATION);
}

void CUMControllerDlg::OnReloadInstantHook()
{
	// 1. Get selected process entry's PID and NT path (same logic as OnSetInstantHook)
	int nItem = m_ProcListCtrl.GetNextItem(-1, LVNI_SELECTED);
	if (nItem == -1) return;
	PROC_ITEMDATA packed = (PROC_ITEMDATA)m_ProcListCtrl.GetItemData(nItem);
	DWORD pid = PID_FROM_ITEMDATA(packed);
	...
	// 3. Defensive check: instant hook must already be enabled
	if (!RegistryStore::HasInstantHookEntry(processFnvHash)) {
		MessageBox(L"Instant Hook is not enabled for this process.", L"Error", MB_ICONERROR);
		return;
	}

	// 4. Read hookSeqPath from registry
	std::wstring hookSeqPath;
	{
		std::vector<std::tuple<unsigned long long, unsigned long long, std::wstring>> entries;
		if (!RegistryStore::ReadInstantHookList(entries)) {
			MessageBox(L"Failed to read Instant Hook registry entries.", L"Error", MB_ICONERROR);
			return;
		}
		for (auto& entry : entries) {
			if (std::get<0>(entry) == processFnvHash) {
				hookSeqPath = std::get<2>(entry);
				break;
			}
		}
	}
	if (hookSeqPath.empty()) {
		MessageBox(L"Failed to retrieve hookseq path from registry.", L"Error", MB_ICONERROR);
		return;
	}
