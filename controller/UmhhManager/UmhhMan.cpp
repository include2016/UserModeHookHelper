...
static void PrintUsage() {
	std::wcout << L"Usage:\n"
		L"  UmhhMan process -a name1[,name2,...]   Append blocked process names\n"
		L"  UmhhMan process -d name1[,name2,...]   Delete process names\n"
		L"  UmhhMan process -c                    Clear all process names\n"
		L"  UmhhMan process -l                    List process names\n"
		L"  UmhhMan dll     -a name1[,name2,...]   Append dll names\n"
		L"  UmhhMan dll     -d name1[,name2,...]   Delete dll names\n"
		L"  UmhhMan dll     -c                    Clear all dll names\n"
		L"  UmhhMan dll     -l                    List dll names\n"
		L"  UmhhMan whitelist -a pePath[,pePath...]  Add PE NT-path hashes to WhitelistHashes\n"
		L"  UmhhMan whitelist -d hexHash[,hexHash...] Delete hashes from WhitelistHashes\n"
		L"  UmhhMan whitelist -l                     List WhitelistHashes\n"
		L"  UmhhMan prot -a name1[,name2,...]        Append protected process names\n"
		L"  UmhhMan prot -d name1[,name2,...]        Delete protected process names\n"
		L"  UmhhMan prot -c                           Clear protected process names\n"
		L"  UmhhMan prot -l                           List protected process names\n"
		L"  UmhhMan hash file <path>                   Compute FNV-1a hash of file content\n"
		L"  UmhhMan hash ntpath <ntPath>               Compute FNV-1a hash of NT path string\n";
	std::wcout << L"\nExtra:\n"
...
int wmain(int argc, wchar_t* argv[]) {
	...
	// Special mode: compute FNV-1a hash
	if (_wcsicmp(category.c_str(), L"hash") == 0) {
		if (argc < 4) { PrintUsage(); return 1; }
		std::wstring hashMode = argv[2]; Trim(hashMode);

		if (_wcsicmp(hashMode.c_str(), L"file") == 0 && argc >= 4) {
			...
			wchar_t out[32];
			swprintf(out, 32, L"%016llx", hash);
			std::wcout << out << std::endl;
			return 0;
		}
		else if (_wcsicmp(hashMode.c_str(), L"ntpath") == 0 && argc >= 4) {
			std::wstring ntPathStr = argv[3];
			unsigned long long h = ComputeNtPathHash(ntPathStr);
			wchar_t out[32];
